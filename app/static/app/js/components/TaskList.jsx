...
class TaskList extends React.Component {
  ...
  loadTaskList() {
    ...
    this.taskListRequest = $.getJSON(this.props.source, (json) => {
      if (json.length === 1) {
      ...
      .fail((jqXHR, textStatus, errorThrown) => {
        this.setState({
          error: interpolate(_("Could not load task list: %(error)s"), {
            error: textStatus,
          }),
        });
      })
  ...
  render() {
    ...
    if (this.state.loading) {
      message = (
        <span>
          {_("Loading...")} <i className="fa fa-sync fa-spin fa-fw"></i>
        </span>
      );
    } else if (this.state.error) {
      message = (
        <span>
          {interpolate(_("Error: %(error)s"), { error: this.state.error })}{" "}
          <a href="javascript:void(0);" onClick={this.retry}>
            {_("Try again")}
          </a>
        </span>
      );
      ...
      <div className="task-list">
        <div>
          ...
          <h4 style={{ color: "#013372" }}>
            {this?.props?.project.tasks.length} projects
          </h4>
        </div>
        <div
          style={{
          ...
          {this.state.tasks
            .filter((t) => {
              const name =
                t.name !== null
                  ? t.name
                  : interpolate(_("Task #%(number)s"), { number: t.id });
              return (
                name
                  .toLocaleLowerCase()
