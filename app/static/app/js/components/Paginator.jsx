...
class Paginator extends React.Component {
  ...
  componentWillUnmount() {
    document.removeEventListener(
      "onProjectListTagClicked",
      this.addTagAndSearch
    );
  }
  ...
  handleSearchChange = (e) => {
    ...
    this.props.handleSearched(e.target.value);
  };
  ...
  sortChanged = (key) => {
    ...
    setTimeout(() => {
      this.props.history.push({
        search: this.getQueryForPage(this.props.currentPage),
      });
    }, 0);
    this.props.handlerefresh();
  };
  ...
  render() {
    ...
    let toolbar = (
      <ul
        className={
          "pagination pagination-sm toolbar " +
          (totalItems == 0 && !searchText ? "hidden " : " ") +
          (totalItems / itemsPerPage <= 1 ? "no-margin" : "")
        }
      >
        <li
          className="btn-group"
        ...
        >
          ...
          <ul className="search-popup">
            <li>
              ...
              />
              <button onClick={this.search} className="btn btn-sm btn-default">
                <i className="fa fa-search"></i>
              </button>
            </li>
          </ul>
        ...
        <li className="btn-group">
          <a
            ...
            aria-expanded="false"
          >
            <i className="fa fa-sort-alpha-down" title={_("Sort")}></i>
          </a>
          <SortPanel
    ...
    if (itemsPerPage && itemsPerPage && totalItems > itemsPerPage) {
      ...
      let pages = [...Array(rangeEnd - rangeStart).keys()].map(
        (i) => i + rangeStart - 1
      );
        ...
        <ul className="pagination pagination-sm">
          ...
          {pages.map((page) => {
            return (
              ...
              >
                <Link to={{ search: this.getQueryForPage(page + 1) }}>
                  {page + 1}
                </Link>
              </li>
            );
    ...
    return [
      <div
        ...
        style={{ width: "-webkit-fill-available", marginTop: "-45px" }}
      >
        {clearSearch}
        {toolbar}
        {paginator}
