...
class Map extends React.Component {
  ...
  typeToIcon = (type) => {
    switch (type) {
      ...
      case "plant":
        return this.props.thermal
          ? "fa fa-thermometer-half fa-fw"
          : "fa fa-seedling fa-fw";
      case "dsm":
      case "dtm":
  ...
  hasBands = (bands, orthophoto_bands) => {
    ...
    for (let i = 0; i < bands.length; i++) {
      if (
        orthophoto_bands.find(
          (b) =>
            b.description !== null &&
            b.description.toLowerCase() === bands[i].toLowerCase()
        ) === undefined
      )
        return false;
    }
      ...
      async.each(
        ...
        (tile, done) => {
          ...
          if (type == "plant") {
            if (
              meta.task &&
              meta.task.orthophoto_bands &&
              meta.task.orthophoto_bands.length === 2
            ) {
            ...
            $.getJSON(metaUrl)
              .done((mres) => {
                ...
                if (
                  ...
                  !this.addedCameraShots[meta.task.id]
                ) {
                  var camIcon = L.icon({
                  ...
                  if (this.props.tiles.length > 1) {
                    // Assign to a group
                    shotsLayer[Symbol.for("meta")].group = {
                      id: meta.task.id,
                      name: meta.task.name,
                    };
                  }
                ...
                if (
                  ...
                  if (this.props.tiles.length > 1) {
                    // Assign to a group
                    gcpLayer[Symbol.for("meta")].group = {
                      id: meta.task.id,
                      name: meta.task.name,
                    };
                  }
    ...
    const UnitsCtrl = Leaflet.Control.extend({
      ...
      onAdd: function () {
        this.container = Leaflet.DomUtil.create(
          "div",
          "leaflet-control-units-selection leaflet-control"
        );
  ...
  handleDeleteAnnotation = (layer) => {
    this.setState({
      annotations: this.state.annotations.filter((l) => l !== layer),
    });
  };
    ...
    return (
      <div style={{ height: "100%" }} className="map">
        ...
        <div className="opacity-slider theme-secondary hidden-xs">
          <div className="opacity-slider-label">{_("Opacity:")}</div>{" "}
          <input
            type="range"
            step="1"
